# $Id: Config.pm,v 1.63 2006/07/02 13:04:35 joern Exp $
...
sub save {
    ...
    print $fh q{# $Id: Config.pm,v 1.63 2006/07/02 13:04:35 joern Exp $},
        "\n";
    print $fh
...
sub _cdrecord_device {
    ...
    if ( $value =~ /^\d+,\d+,\d+$/ ) {
        ...
        return __x( "{value} has not format n,n,n and is no file : NOT Ok",
            value => $value );
    }
}
...
sub _one_of_these {
    ...
    foreach my $val ( @{$lref} ) {
        return __x( "'{value}' is known : Ok", value => $value )
            if $val eq $value;
    }
