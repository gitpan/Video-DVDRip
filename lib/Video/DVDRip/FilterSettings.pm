# $Id: FilterSettings.pm,v 1.10 2005/12/26 13:57:46 joern Exp $
...
sub check_filter_color_mode_ok {
	...
	my $filter = Video::DVDRip::FilterList->get_filter (
...
sub move_instance {
	...
	# no post before pre
	return if not $from_filter->is_pre and $before_instance and $before_instance->queue eq 'pre';
	# no pre at the end
	return if not $from_filter->is_post and not $before_instance;
	# no pre before post, if we don't append it to the pre list
	return if not $append_to_pre and
		  not $from_filter->is_post and $before_instance and
		  $before_instance->queue eq 'post';

	# queue move?
	if ( $from_instance->queue eq 'pre' and
	     ( not $before_instance or $before_instance && $before_instance->queue eq 'post' ) ) {
		$from_instance->set_queue ("post");
		$from_instance->set_value ( option_name => "pre", value => 0, idx => 0);
	} elsif ( $from_instance->queue eq 'post' and
		  ( $append_to_pre or $before_instance && $before_instance->queue eq 'pre') ) {
		$from_instance->set_queue ("pre");
		$from_instance->set_value ( option_name => "pre", value => 1, idx => 0);
	}
...
sub get_filter_config_strings {
	...
	foreach my $instance ( @{$self->filters} ) {
		...
		$filter_name .= "#".$instance->suffix
			if $with_suffixes and $instance->suffix;
...
sub get_max_frames_needed {
	...
	foreach my $instance ( @{$self->filters} ) {
		$max_frames_needed = $instance->get_filter->frames_needed
			if $instance->get_filter->frames_needed >
			   $max_frames_needed;
	}
...
sub new {
	my $class = shift;
	my %par = @_;
	my  ($id, $filter_name, $suffix) =
	@par{'id','filter_name','suffix'};
	...
	$self->set_queue ( 'post' )
		if $self->get_filter->is_post and
		   not $self->get_filter->is_pre_post;

	$self->set_queue ( 'post' )
		if $self->get_filter->is_pre_post and not
		   $self->get_filter
		   	->get_option ( option_name => 'pre' )
			->fields->[0]
			->default;
...
sub get_filter {
	...
	return Video::DVDRip::FilterList
			->get_filter ( filter_name => $self->filter_name );
}

sub set_value {
	my $self = shift;
	my %par = @_;
	my  ($option_name, $idx, $value) =
	@par{'option_name','idx','value'};

	my $filter_name = $self->filter_name;
	my $fields = $self->get_filter
			  ->get_option ( option_name => $option_name )
			  ->fields;
