# $Id: Transcode.pm,v 1.1 2005/10/09 12:04:47 joern Exp $
...
sub new {
	...
	@par{'subtitle_test','cb_update_video_bitrate','cb_exit_program'};
...
sub configure {
	...
	return $self->configure_multipass_with_vbr_audio
	    if $title->has_vbr_audio and
	       $title->tc_multipass and
	       not $title->multipass_log_is_reused;
	...
	$final_task->set_cb_finished (sub {
		...
		if ( $title->tc_exit_afterwards ) {
			...
			my $cb_exit_program = $self->cb_exit_proram;
