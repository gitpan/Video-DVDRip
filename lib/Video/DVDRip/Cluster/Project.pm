# $Id: Project.pm,v 1.34 2005/10/30 12:28:56 joern Exp $
...
sub vob_dir {
    ...
    return $job->node->data_base_dir . "/" . $self->name . "/vob";
}
...
sub create_job_plan {
    ...
    foreach my $psu ( @{ $title->program_stream_units } ) {
        ...
        my $nodes_cnt
            = Video::DVDRip::Cluster::Master->get_master->get_online_nodes_cnt
            + 1;
        ...
        $job
            = Video::DVDRip::Cluster::Job::TranscodeAudio->new( nr => $nr++ );
        push @jobs, $job;
        $job->set_project($self);
        $job->set_psu( $psu->nr );
        ...
        for ( my $i = 0; $i < $chunk_cnt; ++$i ) {
            ...
            foreach my $pass (@pass) {
                $job = Video::DVDRip::Cluster::Job::TranscodeVideo->new(
                    nr => $nr++ );
                push @jobs, $job;
                $job->set_project($self);
                $job->set_pass($pass);
        ...
        $job = Video::DVDRip::Cluster::Job::MergeVideoAudio->new(
            nr => $nr++
        );
        push @jobs, $job;
        $job->set_project($self);
        ...
        if ( $title->is_ogg ) {
            ...
            $job->set_avi_nr(
                $title->audio_tracks->[$vob_nr]->tc_target_track );
            $job->set_vob_nr($vob_nr);
            $job->set_project($self);
            $job->set_psu( $psu->nr );
            $job->set_prefer_local_access(1);
        ...
        if ( keys %{$add_audio_tracks} ) {
            ...
            while ( ( $avi_nr, $vob_nr ) = each %{$add_audio_tracks} ) {
                ...
                $job = Video::DVDRip::Cluster::Job::TranscodeAudio->new(
                    nr => $nr++ );
                push @jobs,          $job;
                push @tc_audio_jobs, $job;
                $job->set_project($self);
                ...
                $job = Video::DVDRip::Cluster::Job::AddAudioMerge->new(
                    nr => $nr++ );
                push @jobs, $job;
                $job->set_avi_nr($avi_nr);
                $job->set_vob_nr($vob_nr);
                ...
                $job->set_prefer_local_access(1);
                $job->set_depends_on_jobs(
                    [ $merge_video_audio_job, $last_job ] );
                $last_job = $job;
            }
        }
    ...
    Video::DVDRip::Cluster::Master->get_master->emit_event( "JOB_PLAN_UPDATE",
        $self->id );
