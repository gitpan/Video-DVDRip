...
sub set_state {
    ...
    my $state = $self->SUPER::set_state(@_);
...
sub new {
    ...
    $self->{id} = Video::DVDRip::Cluster::Master->get_master->get_next_job_id;
...
sub log {
    ...
    my $node_name = $self->node ? $self->node->name : "UNKNOWN";

    $msg =~ s/\n$//;

    $msg = __x(
        "{log_message} on node {node}",
        log_message => $msg,
        node        => $node_name
    );
...
sub commit_job {
    ...
    $master->emit_event( "JOB_UPDATE", $self->id, $self->project->id );
...
sub abort_job {
    ...
    if ( $self->node->state ne 'stopped' ) {
        ...
        # to 'aborted'
        $self->log( "Last output of job was:\n" . $pipe->output_tail );
...
sub line_read {
    ...
    my $master   = Video::DVDRip::Cluster::Master->get_master;
    $master->emit_event( "JOB_PROGRESS_UPDATE", $project->id, $self->nr,
        $self->state, $progress );
    $master->emit_event(
        "NODE_PROGRESS_UPDATE", $self->node->name,
        $self->node_label,      $progress
    );
    1;
}
