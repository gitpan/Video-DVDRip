...
sub write {
	...
	$self->add_field (
		...
		value => $self->format_time (
			time => int ($title->get_transcode_frame_cnt /
	...
	$self->add_field (
		...
		value => sprintf (
			"%d x %d",
			$title->get_transcoded_video_width_height
		)
	);
	$self->add_field (
	...
	$self->add_field (
		...
		value => $Video::DVDRip::deinterlace_filters{$title->tc_deinterlace},
	);
	$self->add_field (
	...
	my $sub_id = 1;
	if ( $title->subtitles ) {
	    foreach my $sub ( sort { $a->id <=> $b->id } values %{$title->subtitles} ) {
		next if not $sub->tc_vobsub and not $sub->tc_render;
		$self->add_head (
			name => "Subtitle $sub_id"
		);
		$self->add_field (
			name 	=> "Id",
			value	=> $sub->id,
	...
	foreach my $audio ( sort { $a->tc_target_track <=> $b->tc_target_track }
		...
		if ( $codec ne 'ac3' and $codec ne 'pcm' ) {
			...
			$self->add_field (
				name  => "Audio filter",
				value => $Video::DVDRip::audio_filters{$audio->tc_audio_filter},
			);
		}
	}
...
sub write_fields {
	...
	print $fh "# Movie information file. Generated by dvd::rip; http://www.exit1.org/dvdrip\n";
