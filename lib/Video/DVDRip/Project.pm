# $Id: Project.pm,v 1.25 2002/09/22 09:41:13 joern Exp $
...
sub vob_title_dir		{ shift->{vob_title_dir}		}
...
sub set_vob_title_dir		{ shift->{vob_title_dir}	= $_[1]	}
...
sub save {
	...
	print $fh q{# $Id: Project.pm,v 1.25 2002/09/22 09:41:13 joern Exp $},"\n";
...
sub load {
	...
	my ($version, $pre) = $data =~ /DVDRip Version (\d+\.\d+)_?(\d+)/;
...
sub check_dvd_in_drive {
	...
	return 1 if $rip_mode ne 'rip' and $rip_mode ne 'dvd';
	
	my $command = "tcprobe -i ".$self->dvd_device.">/dev/null 2>&1";

	system ($command);
	
	if ( $? ) {
		croak "No DVD found at specified location.";
	}
