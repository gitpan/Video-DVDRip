# $Id: Subtitle.pm,v 1.6 2005/12/26 13:57:47 joern Exp $
...
sub build_factory {
	...
	return Gtk2::Ex::FormFactory::VBox->new (
		...
		Gtk2::Ex::FormFactory::VBox->new (
			...
			$self->build_preview_box,
			$self->build_render_box,
			$self->build_vobsub_box,
		    ],
		),
		Gtk2::Ex::FormFactory::Label->new (
		    ...
		    active_cond    => sub {
		        !($self->selected_title &&
		          scalar(keys %{$self->selected_title->subtitles}));
		    },
...
sub build_render_box {
	...
	return Gtk2::Ex::FormFactory::Table->new (
	    object         => "subtitle",
	    active_cond    => sub { $context->get_object_attr("title.subtitle_render_ok") },
		...
		#-- Row #2
	        ...
	        Gtk2::Ex::FormFactory::Label->new (
		    label => __"rows",
		    active_cond => sub { $context->get_object_attr("subtitle.tc_render") },
		),
	        Gtk2::Ex::FormFactory::Label->new (
		    label => __"Gray A/B"." ",
		    ...
		    rules => sub {
		      $_[0] >= 0 && $_[0] <= 255 ||
		      __"Enter value between 0 and 255"
		    },
		),
	        Gtk2::Ex::FormFactory::Label->new (
		    label => __"Preview window"." ",
		    for   => "subtitle.tc_color_b",
		),
	        Gtk2::Ex::FormFactory::Button->new (
...
sub build_vobsub_box {
	...
	return Gtk2::Ex::FormFactory::Table->new (
	    ...
	    content => [
	        ...
	        Gtk2::Ex::FormFactory::Label->new (
		    label => "      ".
		    	     __"Only useful for single-CD-rips",
		),
		
		#-- Row #2
	        Gtk2::Ex::FormFactory::Label->new (
		    label => __"Create later",
		),
	        Gtk2::Ex::FormFactory::CheckButton->new (
	        ...
	        Gtk2::Ex::FormFactory::Label->new (
		    label => "      ".
		    	     __"This considers splitted files correctly",
		),
	    ],
	);
}
...
sub suggest_render_black_bars {
	...
	$self->get_context->get_object("clip_zoom")->generate_preview_images;
	$self->get_context->update_object_widgets("title");
	$self->get_context->update_object_widgets("bitrate_calc");
	$self->get_context->update_object_attr_widgets("subtitle.tc_vertical_offset");
