	    ...
	    content => [
		    ...
		    content => [
			    ...
			    content => [
				    ...
				    bgcolor      => "#ffffff",
				    scale_hook   => sub { $self->get_preview_image_scale },
				    properties   => { xalign => 0, yalign => 0 },
				    with_frame	 => 1,
				    signal_connect => {
				        button_release_event => sub { $self->open_preview( type => "clip1") },
		        		size_allocate        => sub {
    					    return if $self->get_preview_hbox_width  == $_[1]->width*3 and
						      $self->get_preview_hbox_height == $_[1]->height;
					    $self->set_preview_hbox_width($_[1]->width*3);
					    $self->set_preview_hbox_height($_[1]->height);
					    Glib::Source->remove($update_timeout)
						    if $update_timeout;
					    $update_timeout = Glib::Timeout->add (
						100, sub {
						    $_->get_content->[0]->update
							for @{$preview_image_hbox->get_content};
			    ...
			    content => [
				    ...
				    with_frame	 => 1,
				    signal_connect => {
				        button_release_event => sub { $self->open_preview( type => "zoom") },
					realize => sub {
					    $_[0]->window->set_cursor(
					        Gtk2::Gdk::Cursor->new ('GDK_HAND1')
					    );
			    ...
			    content => [
				    ...
				    with_frame	 => 1,
				    signal_connect => {
				        button_release_event => sub { $self->open_preview( type => "clip2") },
					realize => sub {
					    $_[0]->window->set_cursor(
					        Gtk2::Gdk::Cursor->new ('GDK_HAND1')
					    );
...
#		    properties => { column_spacing => 5, row_spacing => 5 },
			...
			Gtk2::Ex::FormFactory::Button->new (
			    ...
			    stock => "gtk-refresh",
			    clicked_hook => sub { $self->generate_preview_images }
			),
			...
			Gtk2::Ex::FormFactory::Button->new (
			    ...
			    clicked_hook => sub { $self->calc_zoom ( height => 1 ) }
			),
			Gtk2::Ex::FormFactory::Button->new (
			    label => __"Calc width",
...
sub get_max_preview_image_size {
	...
	foreach my $type ( "clip1", "zoom", "clip2" ) {
		...
		my ($width, $height);
		if ( not $preview_image_sizes->{$type} or
		     not $preview_image_sizes->{$type}->{$filename} or
		     $preview_image_sizes->{$type}->{$filename}->{mtime} <= $mtime ) {
...
sub get_preview_image_scale {
	...
	my ($max_image_width, $max_image_height) = $self->get_max_preview_image_size;
	...
	$ff = Gtk2::Ex::FormFactory->new (
	  ...
	  content => [
	    Gtk2::Ex::FormFactory::Window->new (
	      title => "dvd::rip ".__x("Preview image: {type}", type => $type),
	      customize_hook => sub {
	        $_[0]->parent->set_resizable(0);
	      },
	      signal_connect => { "destroy" => sub {
	          $self->generate_preview_images if $type ne 'zoom' and
		  				     not $images_generated;
	      }},
	      content => [
	      	$image,
		Gtk2::Ex::FormFactory::DialogButtons->new (
			...
			$self->generate_preview_images unless $type eq 'zoom';
			$images_generated = 1;
			1;
		    },
		),
