# $Id: Transcode.pm,v 1.7 2005/12/26 13:57:47 joern Exp $
...
sub transcode {
    ...
    require Video::DVDRip::Task::Transcode;

    my $task = Video::DVDRip::Task::Transcode->new(
        ui                      => $self,
        project                 => $self->project,
        cb_update_video_bitrate => sub {
            $self->get_context->update_object_attr_widgets(
                "title.tc_video_bitrate");
        },
        cb_exit_proram => sub {
            $self->get_context_object("main")
                ->exit_program(
