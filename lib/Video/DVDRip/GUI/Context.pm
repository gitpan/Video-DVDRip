...
my @BITRATE_PARAMS = (
    "title.tc_container",
    "title.tc_video_bitrate_mode",
    "title.tc_disc_cnt",
    "title.tc_disc_size",
    "title.tc_target_size",
    "title.tc_video_bitrate_range",
    "title.tc_video_bpp_manual",
    ...
    "title.tc_zoom_width",
    "title.tc_zoom_height",
    "title.tc_clip2_top",
    "title.tc_clip2_bottom",
    "title.tc_clip2_left",
    "title.tc_clip2_right",
    "title.tc_start_frame",
    "title.tc_end_frame",
    "audio_track.tc_audio_codec",
    "audio_track.tc_mp3_bitrate",
    "audio_track.tc_mp2_bitrate",
    "audio_track.tc_vorbis_bitrate",
    "audio_track.tc_vorbis_quality",
    "audio_track.tc_vorbis_quality_enable",
    "multi_audio.matrix",
);
...
sub create {
	...
	$context->add_object (
		...
		attr_activity_href => {
		    nptl_ld_assume_kernel => sub {
		        $config->get_value("workaround_nptl_bugs");
		    },
		},
		attr_depends_href => {
	...
	$context->add_object (
		...
		attr_accessors_href => {
			...
			my @titles;
			foreach my $nr ( sort { $a <=> $b } keys %{$self->titles} ) {
			    push @titles, [$nr, __x("DVD title #{nr}", nr => $nr)];
			}
			return \@titles;
		    },
		},
		attr_depends_href => {
		    selected_title    => [ "content.selected_titles", "content.selected_title_nr" ],
	...
	$context->add_object (
		...
		attr_accessors_href => {
			...
			foreach my $audio ( @{$audio_tracks} ) {
				push @audio_list, "$i: ".$audio->info (
					with_target => $with_target_track
				);
				++$i;
			}
			...
			return ["divx4","divx5","xvid","xvid2","xvid3",
		    ...
		    #-- BPP presets
		    tc_video_bpp_manual_presets => [ reverse qw(
			...
			foreach my $subtitle ( sort { $a->id <=> $b->id }
				...
				$selected_label .= $subtitle->info." (".__("render").") | ";
			    } elsif ( $subtitle->tc_vobsub ) {
				$selected_label .= $subtitle->info." (".__("vobsub").") | ";
			    }
			}
			$selected_label =~ s/\| $//;
		...
		attr_depends_href => {
			...
			tc_target_size => [ "title.tc_disc_cnt",
					    "title.tc_disc_size" ],
			...
			subtitles_activated => [ "subtitle.tc_render", "subtitle.tc_vobsub" ],
		},
	);
	...
	$context->add_object (
		...
		attr_accessors_href => {
			...
			tc_mp3_samplerate_presets => [
			  24000, 44100, 48000,
			],
			tc_vorbis_bitrate_presets => [
			  64, 96, 128, 160, 192, 224, 256, 320, 384
			],
			tc_vorbis_samplerate_presets => [
			  24000, 44100, 48000,
			],
			tc_audio_codec     => sub { $audio_codecs{shift->tc_audio_codec} },
			set_tc_audio_codec => sub { $_[0]->set_tc_audio_codec($audio_codecs{$_[1]}) },
			...
			tc_vorbis_quality_presets => [
			    -1,0,1.00,2.00,3.00,4.00,5.00,6.00,7.00,8.00,9.00,10.00
			],
		},
		attr_activity_href => {
	...
	$context->add_object (
		...
		attr_depends_href => {
			sheet => [
			    "content.selected_title_nr",
			    "content.selected_titles",
