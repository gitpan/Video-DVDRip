...
sub open {
	...
	my $transcode_pipe = Video::DVDRip::GUI::Pipe->new (
		command 	=> $command,
		need_output	=> 0,
		cb_finished	=> sub {
...
sub apply_filter_settings {
	...
	my $config_strings =
		$title->tc_filter_settings
		      ->get_filter_config_strings ( with_suffixes => 1 );
	...
	foreach my $config ( @{$config_strings} ) {
		...
		if ( $config->{enabled} ) {
			$transcode_remote->enable_filter (
				filter => $config->{filter}
			);
		} else {
			$transcode_remote->disable_filter (
				filter => $config->{filter}
			);
		}
	}
