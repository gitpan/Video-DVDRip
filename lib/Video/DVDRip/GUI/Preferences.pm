# $Id: Preferences.pm,v 1.4 2005/12/26 13:57:47 joern Exp $
	...
	for ( my $i=0; $i < @{$config_object->order}; ) {
		...
		foreach my $item_name ( @{$order} ) {
			...
			if ( $item_def->{type} eq 'string' ) {
				...
				warn "Unknown config type '$item_def->{type}' for '$item_name'";
			}
...
sub build_config_check_results {
	...
	return Gtk2::Ex::FormFactory::VBox->new (
	    ...
	    content => [
			...
			$tag_table->add ( $self->create_text_tag ( "good_value",
				foreground => "#3f7c3d",
				weight     => 600,
			));
			$tag_table->add ( $self->create_text_tag ( "bad_value",
				foreground => "#ff0000",
				weight     => 600,
			));
...
sub check_params {
	...
	foreach $page ( @pages ) {
		...
		foreach my $option ( @{$options} ) {
			$buffer->insert ($iter, $config_object->config->{$option}->{label}.": ");
			my $result;
			$method = "test_$option";
