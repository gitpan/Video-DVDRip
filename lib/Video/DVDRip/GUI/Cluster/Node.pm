# $Id: Node.pm,v 1.15 2005/12/26 13:57:47 joern Exp $
...
sub open_window {
	...
	$node_ff = Gtk2::Ex::FormFactory->new (
	    ...
	    content   => [
		    ...
		    content => [
		        $self->build_node_form,
			$self->build_node_test_result,
			$self->build_buttons
		    ],
		),
	    ],
...
sub build_node_form {
	...
	return Gtk2::Ex::FormFactory::Form->new (
	    ...
	    content => [
		    ...
		    label => __"Hostname",
		    tip   => __"Network hostname of this node. Defaults to node name.",
		    rules => sub { $_[0] =~ /^[a-z0-9_.-]+$/i ?
		    		   undef : __"No valid hostname" }
		),
	        Gtk2::Ex::FormFactory::Entry->new (
		    ...
		    tip   => __"You can specify additional transcode options for this ".
		    ...
		    tip   => __"Usually you leave this empty (defaults to 'ssh -x'), ".
		...
		Gtk2::Ex::FormFactory::HBox->new (
		    content => [
			    ...
			    stock  => "gtk-network",
			    expand => 0,
			    tip    => __"This triggers a simple test connection to the node, ".
			    ...
			    active_depends => "cluster_node_gui.node_test_timeout",
			),
		    ],
		),
	    ],
...
sub node_test {
	...
	my $timeout = Glib::Timeout->add (
		25,
		sub { $gtk_progress->pulse; 1; }
	);

	$self->set_node_test_timeout($timeout);

	$context->update_object_attr_widgets("cluster_node_gui.node_test_timeout");
...
sub node_test_finished {
	...
	my $master_result = $self->master->get_master_node->test_result;
	my $context       = $self->cluster_ff->get_context;
	...
	if ( $node_content ne $master_content ) {
		...
		$node_content   = "<b><u>".__("Node's data directory")."</u></b>\n".
				  $node_content;
		$master_content = "<b><u>".__("Masters's data directory")."</u></b>\n".
				  $master_content;
	...
	if ( $node_tc ne $master_tc) {
		$node_tc   = "<b><u>".__("Node's transcode")."</u></b>\n".
			     $node_tc;
		$master_tc = "<b><u>".__("Masters's transcode")."</u></b>\n".
			     $master_tc;
		$proxy->set_attrs({
			"res_transcode"         => $not_ok,
	...
	if ( $node_result->{write_test} =~ /SUCCESS/ ) {
		$proxy->set_attrs({
			"res_write_access"         => $ok,
			"res_write_access_details" => __"Node can write to data directory",
		});
	} else {
		$proxy->set_attrs({
			"res_write_access"         => $not_ok,
			"res_write_access_details" => trunc($node_result->{write_test}),
		});
	}
