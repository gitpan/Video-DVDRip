# $Id: Control.pm,v 1.25 2005/10/30 12:32:07 joern Exp $
    ...
    $cluster_ff = Gtk2::Ex::FormFactory->new(
        ...
        content   => [
            Gtk2::Ex::FormFactory::Window->new(
                ...
                content => [
                    $self->build_projects_box, $self->build_jobs_box,
                    $self->build_nodes_box,    $self->build_log_box,
                ],
            ),
        ],
    );
...
sub build_jobs_box {
    ...
    Gtk2::SimpleList->add_column_type(
        ...
        attr     => sub {
            ...
            $cell->set( text => $text );
            $cell->set(
                foreground => $state eq 'aborted' ? "#ff0000" : "#000000" );
            $cell->set( weight => $run ? 700 : 500 );
            1;
        },
    );
    ...
    return Gtk2::Ex::FormFactory::VBox->new(
        ...
        content => [
            Gtk2::Ex::FormFactory::List->new(
                ...
                types => [ "int", ("cluster_job_text") x 5 ],
                selection_mode => "single",
                customize_hook => sub {
                    my ($gtk_simple_list) = @_;
                    ( $gtk_simple_list->get_columns )[0]->set( visible => 0 );
                ...
                if ( $line =~ /EVENT\t(.*)/ ) {
                    ...
                    Glib::Idle->add(
                        sub {
                            ...
                            $text_view->scroll_to_iter( $iter, 0.0, 0, 0.0,
                                0.0 );
                            0;
                        }
                    );
...

{
    ...
    sub process_master_event_queue {
        ...
        my $job_slist
            = $self->get_form_factory->get_widget("cluster_gui.jobs_list")
            ->get_gtk_widget;

        my $node_slist
            = $self->get_form_factory->get_widget("cluster.nodes_list")
            ->get_gtk_widget;
...
# print "update_jobs_list: $project_id <> ".$self->selected_project_id->[0]."\n";
             ...
             # FIXME: update of this job only, currently simple the whole list
                next unless $self->selected_project_id;
                $context->update_object_attr_widgets("cluster_gui.jobs_list")
                    if $project_id == $self->selected_project_id->[0];
...
sub add_project {
    ...
    $self->edit_project( project => $cluster_project, );
