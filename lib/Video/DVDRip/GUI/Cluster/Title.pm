# $Id: Title.pm,v 1.11 2005/12/26 13:57:47 joern Exp $
...
sub new {
	my $class = shift;
	my %par = @_;
	my  ($master, $title, $cluster_ff) =
	@par{'master','title','cluster_ff'};
...
sub open_window {
	...
	my $title_ff;
	$title_ff = Gtk2::Ex::FormFactory->new (
...
sub build_title_form {
	...
	return Gtk2::Ex::FormFactory::Form->new (
	    ...
	    content => [
		    ...
		    tip   => __"The smaller the chunks the higher the parallelism, ".
		    	       "but overall bitrate distribution may suffer if set ".
			       "too low",
		    rules => "positive-integer",
		),
	        Gtk2::Ex::FormFactory::YesNo->new (
		    attr  => "cluster_title_edited.with_avisplit",
		    label => __"Split the file after transcoding?",
		),
	        Gtk2::Ex::FormFactory::YesNo->new (
...
sub build_buttons {
	...
	return Gtk2::Ex::FormFactory::DialogButtons->new (
	    clicked_hook_after => sub {
	        my ($button) = @_;
		if ( $button eq 'ok' ) {
		    $self->title->project->create_job_plan;
