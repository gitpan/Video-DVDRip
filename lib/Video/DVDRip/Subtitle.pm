# $Id: Subtitle.pm,v 1.6.2.1 2005/08/14 16:19:49 joern Exp $
...
sub id				{ shift->{id}				}
sub lang			{ shift->{lang}				}
sub preview_images		{ shift->{preview_images}		}
...
sub set_preview_images		{ shift->{preview_images}	= $_[1]	}
...
sub new {
	...
	@par{'tc_vobsub','tc_render','tc_vertical_offset','tc_time_shift'};
	my  ($tc_antialias, $tc_color_a, $tc_color_b, $tc_assign_color_a) =
	@par{'tc_antialias','tc_color_a','tc_color_b','tc_assign_color_a'};
	my  ($tc_assign_color_b, $tc_test_image_cnt, $title, $tc_color_manip) =
	@par{'tc_assign_color_b','tc_test_image_cnt','title','tc_color_manip'};
	my  ($tc_postprocess) =
	@par{'tc_postprocess'};

	$tc_preview_img_cnt  = 3          if not defined $tc_preview_img_cnt;
...
sub ps1_file {
	...
	return $self->title->project->snap_dir."/".$self->vobsub_prefix.".ps1";
}

sub vobsub_file_exists {
	my $self = shift;
	
	my $mask = $self->title->avi_dir."/".$self->vobsub_prefix.".{sub,rar}";
	my @files = glob($mask);
